# create a data visualization map that visualizes routes flown by RNO, PDX, BOI, and EUG passengers in 2019
...
db1b <- list.files("C:/Users/alexe/Desktop/Nevada", pattern = "*.csv", full.names = TRUE)
...
# clean the D1B1 data
filter_outbound <- dplyr::filter(db1b_DF, origin %in% c('RNO')) %>%
  ...
  dplyr::mutate(first_stop = trim(first_stop),
...
#ggsave(final_map, file = '~/GitHub/Airlines/Des Moines D1B1 Airline Route Map/reno portland eugene boise route map d1b1.png', width = 17, height = 12, units = c('in'))
